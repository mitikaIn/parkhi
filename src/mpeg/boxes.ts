...
export function parseFtyp(data: Uint8Array): string[] {
  ...
  let cursor = 0;
  if (data.length % 4 != 0) throwLengthMismatchError("ftyp", "data", "% 4", data.length);
  if (data.length < 8) throwLengthMismatchError("ftyp", "data", ">= 8", data.length);

  const majorBrand = decodeAscii(data.subarray(cursor, cursor + 4));
  brands.push(majorBrand);
  cursor += 8;

  while (cursor != data.length) {
    const brand = decodeAscii(data.subarray(cursor, cursor + 4));
    brands.push(brand);
    cursor += 4;
  }
